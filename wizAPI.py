...
class wizAPI:
    ...
    def is_health_low(self):
        ...
        COLOR = (113, 48, 3)
        THRESHOLD = 40
        return not pyautogui.pixelMatchesColor(POSITION[0], POSITION[1], COLOR, tolerance = THRESHOLD)
    ...
    def find_spell(self, spell_name, threshold=0.1, max_tries=2, recapture=True):
        """ 
    ...
    def bagIsOpen(self):
        self.set_active()
        if not pyautogui.locateOnScreen('backpack.png') == None: 
            return True 
        else:
            return False
    ...
    def checkBagFull(self):
        ...
        self.openBag()
        self.wait(0.5)
        if not pyautogui.locateOnScreen('fullBag.png') == None:
            return True 
        else:
            return False

    def quickSell(self):
        if not self.bagIsOpen():
            self.openBag()

        self.click(167, 534) # click quick sell
        self.click(423, 225) # select all 
        self.click(410, 398) # say yes 
        self.click(681, 179) # navigate to next window 
        self.click(240, 176) # click on wall hangings 
        locations = list(pyautogui.locateAllOnScreen('woodKey.png')) # list of all the wooden key locations 
        
        # de-select them from the quick sell 
        for i in locations:
            x_pos = i[0]
            y_pos = i[1]
            self.click(x_pos - 75, y_pos)
        
        self.click(246, 493) # click sell 
        self.click(411, 488) # click "I am sure" 
        self.click(403, 397) # click sell
